 ...
 *
 * Note that not every release runs this. A snapshot is cut only when a
 * release changes documented behaviour — a changed default, a rename, a
 * removal, a deprecation. Purely additive releases get a `_0.3+_` marker in
 * docs/ instead, which is why the versions here are expected to be sparse.
 * See VERSIONING.md rules 1 and 2. This script cannot check that condition,
 * so it is on you.
 *
